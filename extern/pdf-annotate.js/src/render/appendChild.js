...
export function getTranslation(viewport) {
  ...
  const CSS = 96.0;
  const PDF = 72.0;
  const CSS_TO_PDF_UNITS = PDF / CSS;
  const scale = viewport.scale * CSS_TO_PDF_UNITS;
  ...
  switch (viewport.rotation % 360) {
    ...
    case 90:
      x = 0;
      y = (viewport.width / scale) * -1;
      break;
    case 180:
      x = (viewport.width / scale) * -1;
      y = (viewport.height / scale) * -1;
      break;
    case 270:
      x = (viewport.height / scale) * -1;
      y = 0;
      break;
  }
...
function transform(node, viewport) {
  ...
  if (!isFirefox && node.nodeName.toLowerCase() === 'svg') {
    ...
    let path = node.querySelector('#icon_sticker_0615');
    let svg = path.parentNode;

    // Scale width/height
    [node, svg, path, node.querySelector('#icon_sticker_0615')].forEach((n) => {
  ...
  if (element) {
    ...
    Object.keys(annotation).filter((key) => {
      return ['color', 'x', 'y', 'cx', 'cy', 'color', 'documentId', 'lines', 'page',
        'width', 'height', 'class', 'content', 'size', 'rotation', 'r', 'paths', 
        'strokeWidth', 'strokeColor', 'opacity', 'strokeDasharray',
        'fillColor', 'rectangles', 'imageUrl'].indexOf(key) === -1;
    }).forEach((key) => {
