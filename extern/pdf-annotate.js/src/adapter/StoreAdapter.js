...
export default class StoreAdapter {
   ...
   */
  getAllAnnotations(documentId) { abstractFunction('getAllAnnotation'); }
   ...
   */
  __addComment(documentId, pageNumber, annotationId, content, dataString, author) { abstractFunction('addComment'); }
  get addComment() { return this.__addComment; }
  set addComment(fn) {
    this.__addComment = function addComment(documentId, pageNumber, annotationId, content, dataString, author) {
      return fn(...arguments).then((comment) => {
        fireEvent('comment:add', documentId, pageNumber, annotationId, comment, dataString, author);
        return comment;
      });
    };
  }
  ...
  __updateAnnotations(documentId, commentId) { abstractFunction('updateAnnotations'); }
  get updateAnnotations() { return this.__updateAnnotations; }
  set updateAnnotations(fn) {
    this.__updateAnnotations = function updateAnnotations(documentId, annotations) {
      fn(...arguments);
      fireEvent('annotation:updated', documentId, annotations);
    };
  }
}
