...
export const BORDER_COLOR = '#2367FF';
...
export function getSVGElement(pageId) {
  return document.querySelector(`svg[data-pdf-annotate-page="${pageId}"]`);
}
...
export function modifyFormComment(docId, pageId, target, result) {
  fireEvent('annotation:modifyComment', docId, pageId, target, result);
}

export function addComment(documentId, pageNumber, annotationId, content, dataString, author) {
  return fireEvent('annotation:addComment', documentId, pageNumber, annotationId, content, dataString, author);
}

//
// mouse / touch 관련 이벤트
//
export const EVENT_DRAW_DOWN = 'pointerdown';
export const EVENT_DRAW_MOVE = 'pointermove';
export const EVENT_DRAW_UP = 'pointerup';

export function addTouchActionNone() {
...
export function removeTouchActionNone() {
  let pages = document.querySelectorAll('.page');
  [...pages].forEach((page) => {
    page.style.touchAction = null;
  });
}
...
// 모바일이나 태블릿일 경우 디폴트 폰트 사이즈는 24
export function calcDefaultFontSize() {
...
// 모바일이나 태블릿일 경우 디폴트 폰트 사이즈는 24
export function calcDefaultTextSize() {
  if (md.mobile() || md.tablet()) {
    return 24;
  }
...
// 모바일이나 태블릿일 경우 메모뷰 비율은 2
export function calcMemoViewScale() {
  if (md.mobile() || md.tablet()) {
    return 2;
  }
...
export function setAuthorName(author) {
  _author = author;
}
